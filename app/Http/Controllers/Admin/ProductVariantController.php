...
class ProductVariantController extends Controller
{
  ...
  public function show(ProductVariant $variant)
  {
    ...
    return Inertia::render('admin/variants/show', [
      'variant' => new ProductVariantResource($variant->load(['discounts', 'product'])),
  ...
  public function store(Request $request)
  {
    ...
    if ($product->variants()->where('name', $validated['name'])->exists()) {
      return back()
        ->withErrors(['name' => 'A variant with this name already exists.'])
        ->withInput();
    }
    ...
    if ($firstVariant && !is_null($firstVariant->attributes) && !is_null($cleanAttributes)) {
      ...
      if ($requiredKeys !== $incomingKeys) {
        return back()
          ->withErrors(['attributes' => 'Keys must match: ' . implode(', ', $requiredKeys)])
          ->withInput();
      }
    }
    ...
    $statusMsg =
      $activeCount === 0 && $product->is_published
  ...
  public function update(Request $request, ProductVariant $variant)
  {
    ...
    if ($exists) {
      return back()
        ->withErrors(['name' => 'Name already in use.'])
        ->withInput();
    }
    ...
    return to_route('admin.variants.show', $variant->id)->with('success', $statusMsg);
  }
