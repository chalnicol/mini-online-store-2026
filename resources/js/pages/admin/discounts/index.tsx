...
const DiscountListing = ({
  ...
  const productsOptions: OptionDetails[] = [{ label: 'Create New Discount', value: 'create' }];
  ...
  return (
    <>
      ...
      <AdminSearchBar table="discounts" filters={filters} className="my-3 flex-1" />
      ...
      <div>
        {items.length > 0 ? (
              ...
              >
                ...
                <div className="flex gap-2 p-2">
                    ...
                    <p className="flex items-center gap-x-1.5 text-sm font-bold text-slate-600">
                      <span
                        className={cn(
                          ...
                          item.type == 'fixed' ? 'bg-orange-600' : 'bg-lime-600',
                        )}
                      >
                        {item.type == 'fixed' ? 'F' : 'P'}
                      </span>
                  ...
                  <div className="flex flex-none flex-col gap-1">
                    <p
                      className={cn('aspect-square w-2 rounded-full', item.isActive ? 'bg-green-600' : 'bg-rose-600')}
                    ></p>
                  </div>
                </div>
          ...
          <div className="flex flex-col items-center justify-center rounded border border-gray-300 bg-gray-100 py-10 text-center">
            ...
            <h2 className="text-2xl font-bold text-gray-800">No Discounts found.</h2>
...
DiscountListing.layout = (page: React.ReactNode) => <AdminLayout children={page} />;
