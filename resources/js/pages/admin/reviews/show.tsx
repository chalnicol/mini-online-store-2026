...
const ReviewDetails = ({ review }: { review: Review }) => {
  ...
  const breadcrumbItems: BreadcrumbItem[] = [{ title: 'Reviews', href: '/admin/reviews' }, { title: 'Review Details' }];
      ...
      <div className="mt-4">
        ...
        <div className="my-3 grid grid-cols-1 gap-x-3 gap-y-6 px-3 md:grid-cols-2 lg:grid-cols-3">
          ...
          <AdminDetailCard title="User">
            <Link href={`/admin/users/${review.user.id}`} className="font-semibold text-sky-600 hover:text-gray-500">
              {review.user.name}
            </Link>
          </AdminDetailCard>
          ...
          <AdminDetailCard title="Product/Variant">
            <Link
              href={`/admin/products/variants/${review.variant.id}`}
          ...
          <AdminDetailCard title="Published" className="">
            <CustomButton
              ...
              size="xs"
              onClick={() => handleTogglePublishedStatus(review.id)}
              disabled={loading}
              loading={loading}
              className="w-26"
            />
          ...
          <AdminDetailCard title="Comment" className="md:col-span-2 lg:col-span-3">
            <p className="flex items-start gap-x-1 text-sm">
              <Quote size={10} className="fill-current text-gray-400" />
...
ReviewDetails.layout = (page: React.ReactNode) => <AdminLayout children={page} />;
