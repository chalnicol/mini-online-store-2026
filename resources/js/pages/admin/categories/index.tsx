...
const CategoryListing = ({ allCategories }: { allCategories: Category[] }) => {
  ...
  const { data, setData, errors, hasErrors, reset, clearErrors, post, processing } = useForm({
    name: '',
    parentId: null,
  });
  ...
  const categoriesOptions: OptionDetails[] = [{ label: 'Create New Category', value: 'create' }];
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between border-b border-gray-400 py-1">
        ...
        <MenuOptions pageOptions={categoriesOptions} onOptionsClick={handleOptionsClick} />
      </div>

      <div className="max-w-xl space-y-2">
        {createNew && (
          <div ref={contRef} className="my-3 space-y-1 overflow-hidden rounded border border-gray-400 px-3 py-2 shadow">
            <p className="text-sm font-bold">Create New</p>

            {hasErrors && <PromptMessage type="error" message={errors.name} className="my-2" />}

            <form className="flex items-start gap-x-2" onSubmit={handleSubmit}>
              <input
                ref={nameRef}
                type="text"
                value={data.name}
        ...
        <div className="my-3 max-h-[65dvh] space-y-2 overflow-y-auto pe-2 pb-2">
          <CategoryManagementProvider>
            {allCategories.map((category) => (
              <CategoryListingBranch key={category.id} category={category} />
            ))}
          </CategoryManagementProvider>
        </div>
      </div>
...
CategoryListing.layout = (page: React.ReactNode) => <AdminLayout children={page} />;
