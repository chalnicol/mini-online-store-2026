...
import { BreadcrumbItem, OptionDetails, Product, ProductVariant } from '@/types/store';
...
const ProductDetails = ({ product }: ProductDetailsProps) => {
  ...
  const [toViewVariant, setToViewVariant] = useState<ProductVariant | null>(null);
  ...
  const pageOptions: OptionDetails[] = [
    ...
    { label: 'View Page Details', value: 'view' },
  ];
    ...
    <>
      ...
      <div className="mt-4">
        <div className="flex items-center gap-x-2 border-b border-slate-400 py-1.5 text-gray-900">
          <p className="font-bold lg:text-lg xl:text-xl">{product.name}</p>
        ...
        {errors && <PromptMessage type="error" errors={errors} className="my-2" />}
        ...
        <div className="mt-3 grid grid-cols-1 gap-x-3 gap-y-6 px-3 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <AdminDetailCard title="Category">
            <p className="text-slate-00 text-sm font-semibold tracking-wider uppercase">{product.category.name}</p>
          </AdminDetailCard>

          <AdminDetailCard title="Status">
            <CustomButton
              label={product.isPublished ? 'UNPUBLISHED' : 'PUBLISH'}
          ...
          <AdminDetailCard title="Rating">
            <Rating rating={product.averageRating} numReviews={product.reviews.length} />
          </AdminDetailCard>

          <AdminDetailCard title="Date">
            <p className="text-sm font-semibold">{formatDate(product.createdAt)}</p>
          </AdminDetailCard>

          <AdminDetailCard title="Description" className="sm:col-span-2 lg:col-span-3">
            <p className="text-sm font-semibold">{product.description || 'No description'}</p>
          </AdminDetailCard>

          {/* variants */}
          <AdminDetailCard title="Variants" className="sm:col-span-2 lg:col-span-3">
                    ...
                    <div className="flex items-start px-2 py-1.5 text-gray-600">
                      <div className="flex gap-x-2">
                        ...
                        <div className="flex flex-1 flex-col items-start">
                          ...
                          <p className="space-x-1 text-xs font-semibold text-sky-700">
                            <span className="text-green-700">{formatPrice(variant.calculatedPrice)}</span>
                            <span>&bull;</span>
                            <span className="text-sky-900">{variant.stockQty} in stock</span>
                            <span>&bull;</span>
                          </p>
                        </div>
                      </div>
                      <p
                        className={cn(
                          ...
                          variant.isActive ? 'bg-emerald-500' : 'bg-rose-500',
                        )}
                      ></p>
                    </div>
...
ProductDetails.layout = (page: React.ReactNode) => <AdminLayout children={page} />;
