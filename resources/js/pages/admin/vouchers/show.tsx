			...
			<div className="mt-4">
				...
				<div className="mt-3 grid grid-cols-1 gap-x-3 gap-y-6 px-3 md:grid-cols-2 lg:grid-cols-3">
					...
					<AdminDetailCard title="Type">
						<p className="text-sm font-semibold tracking-widest uppercase">{voucher.type}</p>
					</AdminDetailCard>
					...
					<AdminDetailCard title="Is Personal">
						<p
							className={cn(
								'mt-1 flex aspect-square w-4 items-center justify-center rounded-full text-sm font-semibold text-white',
								voucher.isPersonal ? 'bg-emerald-600' : 'bg-rose-600',
							)}
						>
							{voucher.isPersonal ? <Check size={14} /> : <X size={14} />}
						</p>
					</AdminDetailCard>
					...
					<AdminDetailCard title="Min. Spend">
						<p className="text-sm font-semibold">
							{voucher.minSpend ? formatPrice(voucher.minSpend) : '--'}
						</p>
					</AdminDetailCard>
					...
					<AdminDetailCard title="Expires At">
						<p
							className={cn(
								...
								voucher.isExpired ? 'text-rose-600' : 'text-sky-900',
							)}
						>
