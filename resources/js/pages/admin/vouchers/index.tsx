...
import type { OptionDetails, PaginatedResponse, VoucherDetails } from '@/types/store';
...
const VoucherListing = ({ vouchers, filters }: VoucherListingProps) => {
  ...
  const options: OptionDetails[] = [{ label: 'Create New Voucher', value: 'create' }];
  ...
  const voucherPersonalCls: Record<string, { label: string; cls: string }> = {
    personal: {
      cls: 'bg-orange-500',
      label: 'Pe',
    },
              ...
              {items.map((item) => (
                ...
                >
                  <div className="flex items-start gap-2 p-2">
                    <p
                      className={cn(
                        'mt-1 flex aspect-square items-center rounded-full px-1.5 text-xs font-semibold tracking-widest text-white',
                        voucherPersonalCls[item.isPersonal ? 'personal' : 'public'].cls,
                      )}
                    >
                      {voucherPersonalCls[item.isPersonal ? 'personal' : 'public'].label}
                    </p>
                    ...
                    <div className="flex flex-none flex-col gap-1">
                      <p
                        className={cn('aspect-square h-2 rounded-full', item.isActive ? 'bg-green-500' : 'bg-rose-600')}
                      ></p>
                    </div>
                  </div>
...
VoucherListing.layout = (page: React.ReactNode) => <AdminLayout children={page} />;
