...
import type { Notification as NotifType, PaginatedResponse } from '@/types/store';
...
const Notifications = ({ data }: NotificationsProps) => {
  ...
  const [localNotifications, setLocalNotifications] = useState<NotifType[]>(data.data);

  const [nextPageUrl, setNextPageUrl] = useState<string | null>(data.links.next);
    ...
    if (target && !target.isRead) {
      ...
      patch(`/profile/notifications/${id}/read`, {
        ...
        onError: () => {
          ...
          setLocalNotifications((prev) => prev.map((n) => (n.id === id ? { ...n, isRead: false } : n)));
        },
      });
    }
  };
  ...
  const handleLoadMore = () => {
    ...
    router.get(
      ...
      {},
      {
        ...
        onSuccess: (page) => {
          const incoming = page.props.data as PaginatedResponse<NotifType>;
          setLocalNotifications((prev) => [...prev, ...incoming.data]);
  ...
  const handleConfirmDelete = () => {
    ...
    destroy(`/profile/notifications/${toDelete.id}`, {
      onSuccess: () => {
        setLocalNotifications((prev) => prev.filter((n) => n.id !== toDelete?.id));
        setToDelete(null);
        setClicked(null);
      },
      preserveScroll: true,
    ...
    <>
      ...
      <div className="flex items-center space-x-2">
        ...
        <CustomButton
          ...
          disabled={processing || isRefreshing || isLoadingMore || unreadNotifications.length === 0}
        ...
        <CustomButton
          ...
          color="danger"
          label="Delete All"
          disabled={processing || isRefreshing || isLoadingMore || localNotifications.length === 0}
      ...
      {localNotifications.length > 0 ? (
          ...
          {nextPageUrl && (
            <div className="mt-4 flex justify-center">
              <CustomButton
                ...
                label={isLoadingMore ? 'Loading...' : 'Load More Notifications'}
        ...
        <div className="mt-3 flex flex-col items-center justify-center py-10 text-center">
          ...
          <h2 className="text-2xl font-bold text-gray-800">No notifications found.</h2>
          <p className="mb-6 text-gray-500">You're all caught up! Check back later for updates.</p>
        </div>
      )}
...
Notifications.layout = (page: React.ReactNode) => <ProfileLayout children={page} />;
