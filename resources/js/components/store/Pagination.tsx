...
const Pagination = ({ meta, type = 'simple', className }: PaginationProps) => {
  ...
  const getFilteredLinks = () => {
    ...
    return allLinks.filter((link, index) => {
      ...
      const isWithinWindow = labelAsNumber >= currentPage - windowSize && labelAsNumber <= currentPage + windowSize;
        ...
        {type === 'simple' && (
          <>
            <Link
              ...
              preserveScroll
              className={cn('block rounded border bg-sky-900 px-3 py-1 font-bold text-white transition', {
                'pointer-events-none border-gray-200 bg-gray-200 text-gray-400': !prevLink?.url,
              })}
            >
              Previous
            </Link>
            <Link
              ...
              preserveScroll
              className={cn('block rounded border bg-sky-900 px-3 py-1 font-bold text-white transition', {
                'pointer-events-none border-gray-200 bg-gray-200 text-gray-400': !nextLink?.url,
              })}
            >
              Next
            </Link>
        ...
        {type === 'advanced' && (
          <>
            {filteredLinks.map((link, i) => {
              ...
              const showDots = i > 1 && !isNaN(currentLabel) && !isNaN(prevLabel) && currentLabel > prevLabel + 1;
                ...
                <div key={i} className="flex items-center gap-x-1.5">
                  ...
                  <Link
                    ...
                    className={cn(
                      ...
                      {
                        'pointer-events-none cursor-default border-sky-800 bg-sky-900 text-white': link.active,
                        'pointer-events-none cursor-default opacity-50': !link.url && !link.active,
                      },
                    )}
                  >
                    {getLabel(link.label)}
