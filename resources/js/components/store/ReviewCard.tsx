...
const ReviewCard: React.FC<ReviewCardProps> = ({ review, size = 'md', className }) => {
  // console.log(review);
  const cls = 'border-t border-gray-300 py-1.5 last:border-b even:bg-gray-100';
  ...
  return (
    <>
      ...
      {size === 'sm' && (
        <div className={cn('flex items-center gap-x-2', cls, paddingCls[size], className)}>
          ...
          <div className="space-y-0.5">
            ...
            <div className="mt-2 flex items-start gap-x-1">
              <Quote size={10} className="fill-current text-gray-500" />
              <p>{review.comment}</p>
            </div>
          </div>
        </div>
