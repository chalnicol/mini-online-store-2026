...
const PromptMessage: React.FC<PromptMessageProps> = ({
  ...
  return (
    <div className={`rounded-e border-l-4 px-3 py-2 ${containerClass[type]} ${className}`}>
      {hasFieldErrors && (
        <div className={`space-y-2 ${detailClass[type]}`}>
          {Object.entries(errors).map(([key, messages]) => (
            <div key={key} className="leading-normal">
              <span className={`${labelTextClass[type]} text-xs font-bold uppercase`}>{key.replace('_', ' ')} :</span>
      ...
      {message && <p className={`${messageTextClass[type]} text-sm font-semibold`}>{message}</p>}
    </div>
  );
};
