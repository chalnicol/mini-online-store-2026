...
const containsSelected = (category: Category, selectedId: number | null): boolean => {
  if (!selectedId) return false;
  if (category.id === selectedId) return true;
  return category.children?.some((child) => containsSelected(child, selectedId)) ?? false;
};
...
const CategoryPickerBranch = ({
  ...
  const [openChildId, setOpenChildId] = useState<number | null>(() => {
    const activeChild = category.children?.find((child) => containsSelected(child, selected));
    ...
    <div>
      <button
        className={cn(
          ...
          isSelected ? 'bg-sky-900 font-bold text-white hover:bg-sky-800' : '',
          isOpen && !isSelected ? 'bg-gray-200 font-semibold text-gray-700 hover:bg-gray-100' : '',
        )}
        disabled={isSelected}
        onClick={() => (hasChildren ? toggleOpen() : onSelect(category))}
      >
        <p className="flex-1 px-3 py-1 text-left">
          {category.name} {hasChildren && `(${category.children.length})`}
        </p>
        {hasChildren && (
...
const CategoryPicker = ({
  ...
  const [openCategoryId, setOpenCategoryId] = useState<number | null>(() => {
    const activeTop = categories.find((cat) => containsSelected(cat, selected));
      ...
      <div className="mb-2 max-h-[60dvh] space-y-2 overflow-y-auto bg-white">
        {categories.map((category) => (
          <CategoryPickerBranch
            ...
            toggleOpen={() => setOpenCategoryId((prev) => (prev === category.id ? null : category.id))}
          />
        ))}
      </div>
    </div>
