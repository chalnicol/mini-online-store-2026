import type { CustomDeliveryTimeDetails, DeliveryType, DeliveryTypeDetails } from '@/types/store';
...
const DeliveryTypeCard: React.FC<DeliveryTypeCardProps> = ({ t, timeData, deliveryType, onClick, onEdit }) => {
  ...
  return (
    <div
      className={`group relative flex flex-1 flex-col rounded border p-2 transition-all duration-200 ${
        isSelected ? 'border-sky-800 bg-gray-50' : 'border-gray-300 bg-white hover:border-sky-800 hover:shadow-sm'
      }`}
    >
      <button
        type="button"
      ...
      >
        <div className="absolute top-1 right-1">
          <Circle size={12} className={`${isSelected ? 'fill-current text-sky-900' : 'text-gray-400'}`} />
        </div>
        ...
        <div className="flex w-full flex-col items-start pr-6">
          ...
          <p className="mt-1 text-sm leading-tight text-gray-500">{t.description}</p>
        </div>
      </button>
          ...
          <div className="mt-2 flex w-full items-start border-t border-gray-300 pt-2 text-sm">
            <div className="flex-1 space-y-1">
              <div className="flex gap-x-1 font-semibold">
                ...
                <span className="text-gray-700">{timeData ? formatShortDate(timeData.date) : '-'}</span>
              </div>
              <div className="flex gap-x-1 font-semibold">
                <span className="flex min-w-12 items-center bg-gray-200 px-2 text-xs text-gray-600 uppercase">
                  Time
                </span>
                <span className="text-gray-700">{timeData ? formatTo12Hour(timeData.time) : '-'}</span>
              </div>
            </div>
            {/* <button
                            type="button"
