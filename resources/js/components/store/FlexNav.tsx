...
const FlexNav = ({ className, parentPath, navItems }: FlexNavProps) => {
  ...
  const activeItem = [...navItems].sort((a, b) => b.href.length - a.href.length).find((item) => isActive(item.href));
  ...
  return (
    <div className={className}>
      {/* Mobile View */}
      <div ref={parentMenuRef} className="relative block rounded bg-sky-900 md:hidden">
        <button
        ...
        >
          ...
          <ChevronDown
            size={18}
            className={cn('ms-auto transition-transform duration-300', showMenu ? 'rotate-180' : '')}
          />
        </button>
