...
const SortAndView: React.FC<SortAndViewProps> = ({ className, itemsCount = 0 }) => {
  const {
    view,
    changeView,
    sortOrder,
  ...
  return (
    <div className={`flex flex-col gap-x-4 gap-y-2 text-xs md:flex-row md:items-center ${className}`}>
      {(searchTerm || selectedCategory) && (
        <div className="flex flex-wrap gap-x-2 gap-y-1">
          {searchTerm && (
            <div className="flex overflow-hidden rounded border bg-sky-900 text-white shadow">
              <p className="flex items-center px-2 py-1 font-semibold">Search: {searchTerm}</p>
              <button
          ...
          {selectedCategory && (
            <div className="flex overflow-hidden rounded border bg-sky-900 text-white shadow">
              <p className="flex items-center px-2 py-1 font-semibold">Category: {selectedCategory}</p>
              <button
                className="cursor-pointer border-s border-white px-1 hover:bg-sky-800"
                onClick={() => {
                  setSelectedSlug('');
