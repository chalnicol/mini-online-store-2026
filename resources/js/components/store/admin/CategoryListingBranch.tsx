...
const CategoryListingBranch = ({ category }: { category: Category }) => {
	...
	const generatedOptions = useMemo(() => {
		...
		const options: OptionDetails[] = [
			{ label: 'View Category', value: 'view' },
	...
	const handleOptionClick = (value: number | string | null) => {
		switch (value) {
			case 'view':
				setMoveId(null);
				router.get(`/admin/categories/${category.id}`);
				break;
			case 'edit':
				setMoveId(null);
	...
	if (toDeleteId == category.id) {
		return (
			...
			>
				<p className="flex-1 font-semibold text-rose-600">{category.name}</p>

				<div className="flex items-center gap-x-2 border-t border-gray-300 pt-1.5 sm:border-0 sm:pt-0">
					<p className="text-sm font-semibold text-rose-400">Confirm delete?</p>
			...
			>
				<button
					className={cn(
						...
						hasChildren ? 'cursor-pointer group-hover:bg-gray-100' : '',
						isOpen ? 'bg-gray-200' : '',
					)}
					...
					{hasChildren && (
						<>
							{category.children.map((subCat) => (
								<CategoryListingBranch key={subCat.id} category={subCat} />
							))}
						</>
					)}
				</div>
