...
const CategoryBranch: React.FC<{
  ...
  const { selectedCategorySlug, setSelectedSlug, expandedId, setExpanded } = useFilterSearch();
  ...
  const isParentOfExpanded = useMemo(() => {
    ...
    const checkChildren = (children: Category[]): boolean => {
      return children.some((child) => child.id === expandedId || (child.children && checkChildren(child.children)));
    };
  ...
  return (
    <div className="flex w-full flex-col">
      <div className={`flex items-stretch overflow-hidden rounded border ${contClass()}`}>
