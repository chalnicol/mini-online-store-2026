...
const ProductDetails: React.FC<ProductDetailsProps> = ({ product }) => {
  ...
  const [selectedAttributes, setSelectedAttributes] = useState<Record<string, string>>(() => {
    if (initialVariantId) {
      const found = variants.find((v) => String(v.id) === initialVariantId);
  ...
  const activeVariant = useMemo(() => {
    const found = variants.find((v) =>
      Object.entries(selectedAttributes).every(([key, value]) => String(v.attributes[key]) === value),
    );
      ...
      <div className="mt-4 mb-6 flex flex-col gap-x-4 gap-y-4 md:flex-row">
              ...
              {variants.map((variant) => (
                <button
                  ...
                  className={`aspect-square w-11 flex-none cursor-pointer overflow-hidden rounded border border-gray-300 bg-gray-100 shadow-sm hover:border-gray-400 ${
                    activeVariant.id === variant.id ? 'border-sky-900 ring-1 ring-sky-900' : ''
                  }`}
                  onClick={() => handleThumbnailClick(variant)}
                >
                  <img src={getImageUrl(variant.imagePath)} alt="" className="h-full w-full object-contain" />
                </button>
              ))}
            </div>
          )}
        ...
        <div className="flex-1 space-y-6">
          <div className="space-y-4">
            <h2 className="text-lg font-semibold md:text-xl lg:text-2xl xl:text-3xl">{product.name}</h2>
            <div className="flex items-baseline gap-x-3">
              <p className="text-2xl font-bold text-red-700">{formatPrice(activeVariant.calculatedPrice)}</p>
              {activeVariant.calculatedPrice < activeVariant.price && (
                <p className="font-semibold text-gray-400 line-through">{formatPrice(activeVariant.price)}</p>
              )}
            </div>
            <Rating rating={product.averageRating} numReviews={product.reviewsCount} />
          </div>
            ...
            {!isSingleVariant && (
              <>
                ...
                {attributeKeys.map((key) => {
                  ...
                  return (
                    <FlexDetail label={`Select ${key}`} key={key}>
            ...
            <FlexDetail label="Quantity">
              <div className="space-y-0.5">
                ...
                {activeVariant.stockQty === 0 ? (
                  ...
                  <span className="text-sm text-sky-900">{activeVariant.stockQty} available</span>
                )}
              </div>
            </FlexDetail>
          </div>
          ...
          <div className="flex w-full gap-x-2 md:w-2/3">
            <CustomButton
              ...
              disabled={activeVariant.stockQty === 0 || processing}
        ...
        <BaseModal>
          <div className="mt-3 w-full max-w-md overflow-hidden rounded bg-white shadow-lg">
            <div className="relative px-4 py-3">
              ...
              <p className="font-semibold text-gray-600">Item added to cart successfully!</p>

              <div className="mt-2 flex items-center gap-3 rounded bg-gray-50 p-2 text-sm">
                <img src={getImageUrl(activeVariant.imagePath)} className="h-10 w-10 rounded object-contain" />
                <div>
                  <p className="font-bold text-gray-700">{product.name}</p>
                  <p className="text-xs text-gray-500">{activeVariant.name}</p>
                </div>
              </div>
